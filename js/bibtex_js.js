...
function BibtexDisplay() {
  ...
  this.fixAuthor = function(string) {
    //Mendeley author format: lastname, first name
    var authorArray = string.split(", ");
    var newString = authorArray[1] + " " + authorArray[0];
  ...
  this.displayAuthor = function(string){
  	...
  	var newString = this.fixAuthor(arrayString[0]);
  	for (i = 1; i < arrayString.length; i++) {
  	  if(i+1>=arrayString.length) {
  	    newString += ", and " + this.fixAuthor(arrayString[i]);
  	  } else {
	    newString += ", " + this.fixAuthor(arrayString[i]);
	  }
	}
	return newString;
  }
